...
function tambahUser($data) {
	...
	if ( $email_user_cek > 0 ) {
		...
		// query insert data
		$query = "INSERT INTO user VALUES ('', '$user_nama', '$user_no_hp', '$user_alamat', '$user_email', '$user_password', '$user_create', '$user_level', '', '', '$user_status', '$user_cabang', '$user_cabang')";
...
function tambahBarang($data) {
	...
	$barang_lokasi_rak  = $data["barang_lokasi_rak"];
	...
	if ( $barang_kode_cek > 0 ) {
		...
		// query insert data
		$query = "INSERT INTO barang VALUES ('', '$barang_kode', '$barang_kode_slug', '$barang_kode_count', '$barang_nama', '','$barang_harga', '$barang_stock', '$barang_tanggal', '$kategori_id', '$kategori_id', '$satuan_id', '$satuan_id', '$barang_deskripsi', '$barang_option_sn', '$barang_status', '', '$barang_lokasi_rak', '$barang_cabang')";
		mysqli_query($conn, $query);
...
function editBarang($data) {
	...
	$query = "UPDATE barang SET 
				...
				barang_lokasi_rak = '$barang_lokasi_rak'
				WHERE barang_id   = $id
				";
	mysqli_query($conn, $query);
	return mysqli_affected_rows($conn);
...
function tambahKeranjangBarcode($data) {
	...
	$barang 	= mysqli_query( $conn, "select barang_id, barang_nama, barang_harga_beli, barang_harga, barang_stock, barang_kode_slug, barang_option_sn, barang_status from barang where barang_kode = '".$barang_kode."' && barang_cabang = ".$keranjang_cabang." ");
	...
	$keranjang_sn       		= 0;
	$barang_status 				= $br["barang_status"];
    	...
    	$idBarang = mysqli_fetch_array($idBarang);
   		...
   		if ( $keranjang_qty_stock >= $barang_stock ) {
			...
			';
	   		...
	   		if ( $barang_status == 0 || $barang_status == 2 ) {
				echo '
					<script>
							alert("Produk TIDAK BISA DITAMBAHKAN Karena Status bukan untuk dijual!! Coba cek kembali di data barang status produk tersebut");
							document.location.href = "beli-langsung";
					</script>
				'; exit();
			}
...
function tambahKendaraan($data) {
	global $conn;
	// ambil data dari tiap elemen dalam form
	$kendaraan_nopol     	= strtoupper(htmlspecialchars($data["kendaraan_nopol"]));
	$kendaraan_pemilik     	= htmlspecialchars($data["kendaraan_pemilik"]);
	$kendaraan_merek    	= htmlspecialchars($data["kendaraan_merek"]);
	$kendaraan_tipe   		= htmlspecialchars($data["kendaraan_tipe"]);
	$kendaraan_jenis   		= htmlspecialchars($data["kendaraan_jenis"]);
	$kendaraan_tahun_buat   = htmlspecialchars($data["kendaraan_tahun_buat"]);
	$kendaraan_tahun_rakit  = htmlspecialchars($data["kendaraan_tahun_rakit"]);
	$kendaraan_silinder   	= htmlspecialchars($data["kendaraan_silinder"]);
	$kendaraan_warna   		= htmlspecialchars($data["kendaraan_warna"]);
	$kendaraan_no_rangka   	= htmlspecialchars($data["kendaraan_no_rangka"]);
	$kendaraan_no_mesin   	= htmlspecialchars($data["kendaraan_no_mesin"]);
	$kendaraan_keterangan   = htmlspecialchars($data["kendaraan_keterangan"]);
	$kendaraan_date     	= date("Y-m-d");
	$kendaraan_datetime   	= date("d F Y g:i:s a");
	$kendaraan_cabang  		= htmlspecialchars($data["kendaraan_cabang"]);

	// Cek Email
	$customer_nopol_cek = mysqli_num_rows(mysqli_query($conn, "select * from kendaraan where kendaraan_nopol = '".$kendaraan_nopol."' && kendaraan_cabang = $kendaraan_cabang "));

	if ( $customer_nopol_cek > 0 ) {
		echo "
			<script>
				alert('No. Pol Sudah Terdaftar');
			</script>
		";
	} else {
		// query insert data
		$query = "INSERT INTO kendaraan VALUES ('', '$kendaraan_nopol', '$kendaraan_pemilik', '$kendaraan_merek', '$kendaraan_tipe', '$kendaraan_jenis', '$kendaraan_tahun_buat', '$kendaraan_tahun_rakit', '$kendaraan_silinder', '$kendaraan_warna', '$kendaraan_no_rangka', '$kendaraan_no_mesin', '$kendaraan_keterangan', '$kendaraan_date', '$kendaraan_datetime', '', '$kendaraan_cabang')";
		mysqli_query($conn, $query);
...
function editKendaraan($data){
	...
	$kendaraan_keterangan   = htmlspecialchars($data["kendaraan_keterangan"]);
		...
		$query = "UPDATE kendaraan SET 
						kendaraan_nopol     	= '$kendaraan_nopol',
						kendaraan_pemilik     	= '$kendaraan_pemilik',
						kendaraan_merek    		= '$kendaraan_merek',
						kendaraan_tipe   		= '$kendaraan_tipe',
						kendaraan_jenis   		= '$kendaraan_jenis',
						kendaraan_tahun_buat   	= '$kendaraan_tahun_buat',
						kendaraan_tahun_rakit   = '$kendaraan_tahun_rakit',
						kendaraan_silinder  	= '$kendaraan_silinder',
						kendaraan_warna   		= '$kendaraan_warna',
						kendaraan_no_rangka   	= '$kendaraan_no_rangka',
						kendaraan_no_mesin   	= '$kendaraan_no_mesin',
						kendaraan_keterangan   	= '$kendaraan_keterangan'
...
function hapusKendaraan($id) {
	global $conn;
	mysqli_query( $conn, "DELETE FROM kendaraan WHERE kendaraan_id = $id");
...
function updateInvoice($data) {
	...
	$invoice_sub_total    = $invoice_total;
	$invoice_bayar        = htmlspecialchars($data['angka1']);
	$invoice_kembali      = $invoice_bayar - $invoice_sub_total;
...
function tambahServis($data) {
    ...
    $servis_nama = htmlspecialchars($data['servis_nama']);
    $servis_desc = htmlspecialchars($data['servis_desc']);
    $servis_biaya_profit = (float) htmlspecialchars($data['servis_biaya_profit']);
    $persentase_jasa_teknisi = (float) htmlspecialchars($data['servis_biaya_jasa_teknisi']);

    // Hitung nominal jasa teknisi
    $servis_biaya_jasa_teknisi = ($servis_biaya_profit * $persentase_jasa_teknisi) / 100;

    // Hitung total biaya
    $servis_biaya = $servis_biaya_profit - $servis_biaya_jasa_teknisi;
...
// function tambahBiayaJasaServis($nota, $id, $servis_kategori, $servis_nama, $servis_biaya_jasa_teknisi, $servis_biaya_profit, $servis_biaya, $sessionCabang) {
// 	global $conn;

// 	// query insert data
// 	$query = "INSERT INTO data_servis_teknisi VALUES ('', '$nota', '', '$id', '$servis_kategori', '$servis_nama', '$servis_biaya_jasa_teknisi', '$servis_biaya_profit', '$servis_biaya', '', '$sessionCabang')";
// 	mysqli_query($conn, $query);

// 	return mysqli_affected_rows($conn);
// }

function tambahBiayaJasaServis($nota, $id, $servis_kategori, $servis_nama, $persentase_jasa_teknisi, $servis_biaya_profit, $sessionCabang) {
    global $conn;
...
function updateMekanikServis($data){
	global $conn;
	$id = $data["dst_id"];

	$dst_teknisi_id = htmlspecialchars($data['dst_teknisi_id']);

	// query update data
	$query = "UPDATE data_servis_teknisi SET 
						dst_teknisi_id  = '$dst_teknisi_id'
						WHERE dst_id      = $id
				";
...
function editServisTeknisi($data) {
	...
	$ds_kendaraan_id 				= $data['ds_kendaraan_id'];
	$ds_kendaraan_km 				= $data['ds_kendaraan_km'];

	$ds_servis_berkala_waktu_angka			= $data['ds_servis_berkala_waktu_angka'];
	$ds_servis_berkala_waktu_teks 			= $data['ds_servis_berkala_waktu_teks'];

	$tgl1 = date('Y-m-d');
	if ( $ds_servis_berkala_waktu_teks === "days" ) { 
		$ds_servis_berkala = date('Y-m-d', strtotime('+'.$ds_servis_berkala_waktu_angka.' '.$ds_servis_berkala_waktu_teks, strtotime( $tgl1 ))); 
	} elseif ( $ds_servis_berkala_waktu_teks === "month" ) { 
		$ds_servis_berkala = date('Y-m-d', strtotime('+'.$ds_servis_berkala_waktu_angka.' '.$ds_servis_berkala_waktu_teks, strtotime( $tgl1 ))); 
	} elseif ( $ds_servis_berkala_waktu_teks === "year" ) { 
		$ds_servis_berkala = date('Y-m-d', strtotime('+'.$ds_servis_berkala_waktu_angka.' '.$ds_servis_berkala_waktu_teks, strtotime( $tgl1 ))); 
	} else {
		$ds_servis_berkala 					= "0000-00-00";
	}

	$ds_servis_berkala 					    = $ds_servis_berkala;
	$pecah_data 				        	= explode("-",$ds_servis_berkala);
	$ds_servis_berkala_date_time_tgl 		= $pecah_data[2];
	$ds_servis_berkala_date_time_bln 		= $pecah_data[1];
	if ( $ds_servis_berkala_date_time_bln == 1 ) {
		$ds_servis_berkala_date_time_bln = 'January';
	} elseif ( $ds_servis_berkala_date_time_bln == 2 ) {
		$ds_servis_berkala_date_time_bln = 'February';
	} elseif ( $ds_servis_berkala_date_time_bln == 3 ) {
		$ds_servis_berkala_date_time_bln = 'March';
	} elseif ( $ds_servis_berkala_date_time_bln == 4 ) {
		$ds_servis_berkala_date_time_bln = 'April';
	} elseif ( $ds_servis_berkala_date_time_bln == 5 ) {
		$ds_servis_berkala_date_time_bln = 'May';
	} elseif ( $ds_servis_berkala_date_time_bln == 6 ) {
		$ds_servis_berkala_date_time_bln = 'June';
	} elseif ( $ds_servis_berkala_date_time_bln == 7 ) {
		$ds_servis_berkala_date_time_bln = 'July';
	} elseif ( $ds_servis_berkala_date_time_bln == 8 ) {
		$ds_servis_berkala_date_time_bln = 'August';
	} elseif ( $ds_servis_berkala_date_time_bln == 9 ) {
		$ds_servis_berkala_date_time_bln = 'September';
	} elseif ( $ds_servis_berkala_date_time_bln == 10 ) {
		$ds_servis_berkala_date_time_bln = 'October';
	} elseif ( $ds_servis_berkala_date_time_bln == 11 ) {
		$ds_servis_berkala_date_time_bln = 'November';
	} elseif ( $ds_servis_berkala_date_time_bln == 12 ) {
		$ds_servis_berkala_date_time_bln = 'December';
	}
	$ds_servis_berkala_date_time_thn 	= $pecah_data[0];
	$ds_servis_berkala_date_time        = $ds_servis_berkala_date_time_tgl." ".$ds_servis_berkala_date_time_bln." ".$ds_servis_berkala_date_time_thn;
	$ds_servis_berkala_teks_wa			= htmlspecialchars(str_replace("&", "dan", $data['ds_servis_berkala_teks_wa']));
	$ds_servis_berkala_status 			= 1;
	...
	$query = "UPDATE data_servis SET 
				...
				ds_total_sisa_bayar 			= '$ds_total_sisa_bayar',
 				ds_status 						= '$ds_status',
 				ds_servis_berkala               = '$ds_servis_berkala',
				ds_servis_berkala_date_time     = '$ds_servis_berkala_date_time',
				ds_servis_berkala_waktu_angka   = '$ds_servis_berkala_waktu_angka',
				ds_servis_berkala_waktu_teks    = '$ds_servis_berkala_waktu_teks',
				ds_servis_berkala_teks_wa       = '$ds_servis_berkala_teks_wa',
				ds_servis_berkala_status        = '$ds_servis_berkala_status'
				WHERE ds_id 	     			= $id
				";
	mysqli_query($conn, $query);

	$query4 = "UPDATE kendaraan SET
				kendaraan_km 				= '$ds_kendaraan_km'
				WHERE kendaraan_id 			= $ds_kendaraan_id
				";
	mysqli_query($conn, $query4);

	// Cek Data 
	$countHistory = mysqli_query($conn, "SELECT * FROM history_servis_tekinis WHERE hst_nota = $ds_nota && hst_cabang = $ds_cabang  ");
	$countHistory = mysqli_num_rows($countHistory);

	if ( $countHistory < 1 ) {
		$query2 = "INSERT INTO history_servis_tekinis VALUES ('', '$ds_nota', '', '$ds_status', '$hst_date', '$hst_date_time', '$ds_cabang')";				
		mysqli_query($conn, $query2);
	} else {
...
// =========================================== Servis Berkala ============================== //
function editStatusServisBerkala($id) {
	global $conn;
